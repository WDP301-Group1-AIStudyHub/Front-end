...
export interface AdminStorageSnapshot {
  ...
  package: AdminStoragePackage | null
...
export interface AdminTransactionPackageSnapshot {
  code: string
  name: string
  capacityBytes: number
  priceVnd: number
...
export interface AdminPaymentBreakdown {
  [key: string]: { count: number; amountVnd: number }
}
...
export interface AdminPaymentsOverview {
  ...
  byStatus: AdminPaymentBreakdown
  byProvider: AdminPaymentBreakdown
  byPlatform: AdminPaymentBreakdown
  dateFrom: string | null
  dateTo: string | null
}

export interface AdminStorageUserDetail {
  user: AdminUserSummary & { createdAt: string | null }
  storage: AdminStorageSnapshot
  package: AdminStoragePackage | null
