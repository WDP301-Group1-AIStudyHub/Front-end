...
export default function StudyMaterialsListPage() {
  ...
  const [progressData, setProgressData] = useState<
    Record<string, StudyProgressData>
  >({});

  // Filters state
  const [searchTerm, setSearchTerm] = useState("");
  const [typeFilter, setTypeFilter] = useState<"ALL" | "MCQ" | "FLASHCARD">(
    "ALL",
  );
  ...
  const handleOpenCustomise = async (
    type: MaterialType,
    prefillDocId?: string,
    prefillTopicFocus?: string,
  ) => {
    ...
    try {
      ...
      if (prefillDocId && docs) {
        const foundDoc = docs.find(
          (d: any) => d._id === prefillDocId || d.id === prefillDocId,
        );
        if (foundDoc) {
          const subId =
            foundDoc.subjectId ||
            (foundDoc.subject && typeof foundDoc.subject === "object"
              ? foundDoc.subject._id
              : "");
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setMaterials((prev) =>
        prev.filter((m) => m._id !== deleteTargetId && m.id !== deleteTargetId),
      );
    } catch (err: any) {
  ...
  return (
    <PageShell>
        ...
        >
          <CardContent className="flex items-center gap-3 cursor-pointer">
            ...
            <div className="text-sm">
              ...
              <p className="line-clamp-2 mt-0.5 text-muted-foreground">
        ...
        >
          <CardContent className="flex items-center gap-3 cursor-pointer">
            ...
            <div className="text-sm">
              ...
              <p className="line-clamp-2 mt-0.5 text-muted-foreground">
                Generate flip flashcards for active recall and studying
      ...
      {/* Filter Toolbar */}
      <section className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mt-4">
        {/* Search box */}
      ...
      {isLoading ? (
        ...
        <>
          {filteredMaterials.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-12 text-center bg-card border border-border rounded-2xl my-4 space-y-2">
              <HelpCircle className="size-10" strokeWidth={1.5} />
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 mt-2">
              {filteredMaterials.map((mat) => {
                ...
                const isGenerating =
                  mat.status === "PENDING" || mat.status === "GENERATING";
