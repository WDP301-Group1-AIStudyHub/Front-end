...
export default function AdminActivityPage() {
  ...
  const [viewingActivity, setViewingActivity] = useState<SystemActivity | null>(
    null,
  );
  const [isLoading, setIsLoading] = useState(true);
  const [query, setQuery] = useState("");
  const [severityFilter, setSeverityFilter] = useState<
    "all" | SystemActivity["severity"]
  >("all");
  const [typeFilter, setTypeFilter] = useState<"all" | SystemActivity["type"]>(
    "all",
  );
  ...
  const filteredActivities = useMemo(() => {
    ...
    return activities.filter((activity) => {
      const matchesQuery =
        !normalizedQuery ||
        [activity.actor, activity.description, activity.target].some((value) =>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-1.5">
              ...
              <Select
                ...
                onValueChange={(val) =>
                  setSeverityFilter(val as typeof severityFilter)
                }
              >
        ...
        <div className="divide-y divide-border/60">
          {isLoading ? (
            ...
            paginatedActivities.map((activity) => (
              <article
                className="grid gap-4 p-5 transition-colors hover:bg-muted/35 lg:grid-cols-[160px_1fr_120px_110px_90px]"
                key={activity.id}
              >
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">
                        {activity.actor}
                        {!["Account", "Platform"].includes(activity.target) &&
                          ` → ${activity.target}`}
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex items-center">
                  <StatusBadge severity={activity.severity}>
                    {activity.severity}
                  </StatusBadge>
                </div>
              ...
              <div className="bg-muted/40 p-6 border-b border-border/50">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <StatusBadge severity={viewingActivity.severity}>
                      {viewingActivity.severity}
                    </StatusBadge>
                    <StatusBadge severity="info">
              ...
              <div className="p-6 overflow-y-auto space-y-8">
                ...
                <section>
                  <h3 className="text-sm font-semibold text-foreground/80 mb-4 flex items-center gap-2">
                    <User className="size-4 text-primary/70" /> General
                    Information
                  </h3>
                    ...
                    {viewingActivity.entityType && (
                      <div className="bg-muted/20 border border-border/50 rounded-xl p-4">
                        ...
                        <p
                          ...
                          title={viewingActivity.entityId}
                        >
                ...
                <section>
                  <h3 className="text-sm font-semibold text-foreground/80 mb-4 flex items-center gap-2">
                    <Globe className="size-4 text-primary/70" /> Technical
                    Details
                  </h3>
                    ...
                    {viewingActivity.userAgent && (
                      <div className="flex flex-col sm:flex-row gap-2 p-4">
                        <div className="sm:w-[150px] flex items-center gap-2 text-sm font-medium text-muted-foreground shrink-0">
                          <MonitorSmartphone className="size-4" /> Device &
                          Browser
                        </div>
