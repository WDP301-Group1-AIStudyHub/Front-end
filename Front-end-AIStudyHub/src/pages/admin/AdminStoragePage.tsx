...
export default function AdminStoragePage() {
  ...
  const [editingPackage, setEditingPackage] =
    useState<AdminStoragePackage | null>(null);
  const [packageForm, setPackageForm] = useState<PackageFormState | null>(null);
  const [pendingPackageChange, setPendingPackageChange] = useState<{
    userId: string;
    packageId: string;
    packageName: string;
  } | null>(null);
      ...
      showToast({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Unable to load storage management",
      });
    } finally {
      setLoading(false);
    }
  ...
  const openDetail = async (userId: string) => {
    ...
    try {
      setDetail(await getAdminStorageUser(userId));
    } catch (error) {
      showToast({
        tone: "error",
        message:
  ...
  const reconcileUser = async (userId: string) => {
    ...
    try {
      ...
      showToast({
        tone: "success",
        message: `Reconciled. Drift: ${formatBytes(Math.abs(result.reconciliation.drift || 0))}`,
      });
      await load();
  ...
  const reconcileAll = async () => {
    ...
    try {
      ...
      showToast({
        tone: "success",
        message: `Scanned ${result.scanned || 0} users; corrected ${result.corrected || 0}.`,
      });
      await load();
    } catch (error) {
      showToast({
  ...
  const changePackage = async (userId?: string, packageId?: string) => {
    ...
    if (!pendingPackageChange || !packageChangeReason.trim()) {
      showToast({
        tone: "error",
        message: "A reason is required for package changes.",
      });
      return;
    }
    setBusy(true);
    try {
      const result = await assignAdminStoragePackage(
        pendingPackageChange.userId,
        pendingPackageChange.packageId,
        packageChangeReason.trim(),
      );
      showToast({
        tone: "success",
        message: `Package changed to ${result.package?.name || "selected package"}.`,
      });
  ...
  const togglePackage = async (pkg: AdminStoragePackage) => {
    ...
    try {
      ...
      showToast({
        tone: "success",
        message: `${pkg.name} is now ${pkg.isActive ? "inactive" : "active"}.`,
      });
      await load();
    } catch (error) {
      showToast({
  ...
  const savePackage = async () => {
    ...
    if (features.some((feature) => feature.length > 200)) {
      showToast({
        tone: "error",
        message: "Each feature must be 200 characters or fewer.",
      });
      return;
    }
    ...
    try {
      ...
      showToast({
        tone: "success",
        message: `${editingPackage.name} updated.`,
      });
  ...
  const makeDefault = async (pkg: AdminStoragePackage) => {
    setBusy(true);
    try {
      await updateAdminStoragePackage(pkg.id, {
        isDefault: true,
        isActive: true,
      });
      showToast({
        tone: "success",
        message: `${pkg.name} is now the default package.`,
      });
      await load();
    } catch (error) {
      showToast({
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        description="Monitor quota, change valid packages and reconcile actual document usage."
        actions={
          <Button disabled={busy} onClick={() => void reconcileAll()}>
            <RefreshCw /> Reconcile all
          </Button>
        }
      />
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <AdminStatCard
          ...
          value={loading ? "..." : formatBytes(overview?.totalQuotaBytes ?? 0)}
          tone="teal"
        />
        <AdminStatCard
          icon={<HardDrive />}
          ...
          value={
            loading
              ? "..."
              : formatBytes(
                  (overview?.totalUsedBytes ?? 0) +
                    (overview?.totalReservedBytes ?? 0),
                )
          }
          tone="gold"
        />
        <AdminStatCard
          icon={<Settings2 />}
          label="Users at risk"
          value={loading ? "..." : String(overview?.usersAtRisk ?? 0)}
          tone="coral"
        />
      </section>
      <section className="grid gap-6 xl:grid-cols-[1.4fr_0.8fr]">
        <Card>
          <CardHeader>
            <CardTitle>Users and quota</CardTitle>
            <div className="flex flex-col gap-2 sm:flex-row">
              ...
              <select
                ...
                value={status}
                onChange={(event) =>
                  setStatus(event.target.value as StorageAdminStatus | "")
                }
              >
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {loading ? (
                    ...
                    users.map((row) => (
                      <tr className="border-b" key={row.user?.id}>
                        <td className="p-3">
                          <button
                            className="text-left font-medium hover:underline"
                            onClick={() =>
                              row.user && void openDetail(row.user.id)
                            }
                          >
                        ...
                        <td className="p-3">
                          {row.storage.package?.name || "Unknown"}
                          <span className="block text-xs text-muted-foreground">
                            {formatBytes(row.storage.quotaBytes)}
                          </span>
                        </td>
                        <td className="p-3">
                          <StatusBadge
                            severity={row.storage.status.toLowerCase()}
                          >
                            {row.storage.usagePercent}% ·{" "}
                            {formatBytes(
                              row.storage.usedBytes + row.storage.reservedBytes,
                            )}
                          </StatusBadge>
                        </td>
                        <td className="p-3">
                          <div className="flex flex-wrap gap-1">
                            ...
                            <Button
                              ...
                              disabled={busy}
                              onClick={() =>
                                row.user && void reconcileUser(row.user.id)
                              }
                            >
                              Reconcile
                            </Button>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    ...
                    {pkg.isActive ? formatVnd(pkg.priceVnd) : "Inactive"}
                  </p>
                </div>
                <div className="flex flex-wrap justify-end gap-1">
                  ...
                  {!pkg.isDefault ? (
                    <Button
                      size="sm"
                      variant="ghost"
                      disabled={busy || !pkg.isActive}
                      onClick={() => void makeDefault(pkg)}
                    >
                      Default
                    </Button>
                  ) : null}
            ...
            <div className="border-t pt-4">
              <p className="mb-2 text-sm font-medium">Create package</p>
              <div className="grid gap-2 sm:grid-cols-2">
                ...
                <Input
                  type="number"
                  placeholder="Capacity MB"
                  value={packageDraft.capacityMb}
                  onChange={(event) =>
                    setPackageDraft((draft) => ({
                      ...draft,
                      capacityMb: event.target.value,
                    }))
                  }
                />
                <Input
              ...
              <Button
                ...
                disabled={busy || !packageDraft.code || !packageDraft.name}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              <span>{detail.user.fullName} · storage detail</span>
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-4">
              ...
              <div>
                ...
                <p>{formatBytes(detail.storage.usedBytes)}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Reserved</p>
                <p>{formatBytes(detail.storage.reservedBytes)}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Package</p>
                <p>{detail.package?.name || "Unknown"}</p>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <select
                ...
                onChange={(event) =>
                  void changePackage(detail.user.id, event.target.value)
                }
              >
                ...
                {packageOptions.map((pkg) => (
                  <option key={pkg.id} value={pkg.id}>
                    {pkg.name} · {formatBytes(pkg.capacityBytes)}
                  </option>
                ))}
              </select>
              <Button
                variant="outline"
                disabled={busy}
                onClick={() => void reconcileUser(detail.user.id)}
              >
                Reconcile this user
              </Button>
            </div>
                ...
                {detail.recentTransactions.length ? (
                  detail.recentTransactions.map((transaction) => (
                    <div
                      ...
                      key={transaction.orderRef}
                    >
                      <span>
                        {transaction.orderRef} · {transaction.package.name}
                      </span>
                      <StatusBadge severity={transaction.status.toLowerCase()}>
                        {transaction.status}
                      </StatusBadge>
                    </div>
                  ))
            ...
            <p className="text-xs text-muted-foreground">
              Last reconciled:{" "}
              {formatDateTime(detail.storage.lastReconciledAt || undefined)}
            </p>
          </CardContent>
        </Card>
      ) : null}
          ...
          <DialogHeader>
            <DialogTitle>
              {detailLoading
                ? "Loading user detail…"
                : detail
                  ? `${detail.user.fullName} · storage detail`
                  : "Storage detail"}
            </DialogTitle>
            <DialogDescription>
              Quota, usage, assigned package and recent payment transactions.
            </DialogDescription>
          </DialogHeader>
          {detail ? (
            <div className="space-y-5">
              <div className="grid gap-3 sm:grid-cols-4">
                <div>
                  <p className="text-xs text-muted-foreground">Email</p>
                  <p className="break-words">{detail.user.email}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Used</p>
                  <p>{formatBytes(detail.storage.usedBytes)}</p>
        ...
        {editingPackage && packageForm ? (
          <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Update the package settings used for new purchases. Existing
                subscribers keep their current quota.
              </DialogDescription>
            </DialogHeader>
            <form
              className="space-y-5"
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      setPackageForm(
                        (draft) =>
                          draft && { ...draft, capacityMb: event.target.value },
                      )
                    }
                    required
                  />
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      setPackageForm(
                        (draft) =>
                          draft && { ...draft, priceVnd: event.target.value },
                      )
                    }
                    required
                  />
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      setPackageForm(
                        (draft) =>
                          draft && { ...draft, sortOrder: event.target.value },
                      )
                    }
                    required
                  />
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(event) =>
                    setPackageForm(
                      (draft) =>
                        draft && { ...draft, description: event.target.value },
                    )
                  }
                  maxLength={500}
                  rows={3}
                ...
                />
                <p className="text-xs text-muted-foreground">
                  {packageForm.description.length}/500 characters
                </p>
              </div>
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(event) =>
                    setPackageForm(
                      (draft) =>
                        draft && { ...draft, features: event.target.value },
                    )
                  }
                  rows={5}
                  placeholder={
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Enter one feature per line. Empty lines are removed when
                  saved.
                </p>
              </div>
                ...
                <label className="flex items-start gap-3 rounded-lg border p-3 text-sm">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setPackageForm(
                        (draft) =>
                          draft && { ...draft, highlight: checked === true },
                      )
                    }
                  />
                  <span>
                    <span className="block font-medium">Highlight</span>
                    <span className="text-xs text-muted-foreground">
                      Show as a featured plan.
                    </span>
                  </span>
                </label>
                <label className="flex items-start gap-3 rounded-lg border p-3 text-sm">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setPackageForm(
                        (draft) =>
                          draft && { ...draft, isActive: checked === true },
                      )
                    }
                  />
                  <span>
                      ...
                      setPackageForm(
                        (draft) =>
                          draft && {
                            ...
                            isActive: checked === true ? true : draft.isActive,
                          },
                      )
                    }
                  />
                  <span>
                    <span className="block font-medium">Default package</span>
                    <span className="text-xs text-muted-foreground">
                      Used when provisioning a new user.
                    </span>
                  </span>
                </label>
              </div>
            ...
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                <span className="font-medium text-foreground">
                  {pendingPackageChange.packageName}
                </span>{" "}
                to this user. This action is recorded in the activity log.
              </DialogDescription>
            </DialogHeader>
            <form
              className="space-y-4"
            ...
            >
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(event) =>
                    setPackageChangeReason(event.target.value)
                  }
                  maxLength={500}
                  rows={4}
                ...
                />
                <p className="text-xs text-muted-foreground">
                  {packageChangeReason.length}/500 characters
                </p>
              </div>
              <DialogFooter>
                <DialogClose asChild>
                ...
                <Button
                  type="submit"
                  disabled={busy || !packageChangeReason.trim()}
                >
                  {busy ? "Applying…" : "Apply package"}
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
