...
import { listSubjects } from "../services/adminApi";
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <label className="flex flex-col gap-2 text-sm font-medium">
        Subject
        <select
          className="h-10 rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-xs outline-none transition-[color,box-shadow] focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:cursor-not-allowed disabled:opacity-50"
          disabled={disabled}
          onChange={(event) =>
...
export default function NewLibraryPage() {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      void loadDocuments();
      void listSubjects().then(setSubjects).catch(() => setSubjects([]));
    }, 0);
          ...
          {isUploading && (
            <div className="celestial-card tone-surface tone-cyan flex flex-col gap-2 p-4">
              <div className="flex items-center justify-between gap-3 text-sm font-medium">
                ...
                <span className="status-badge status-processing">{uploadProgress}%</span>
              </div>
                        ...
                        <TableCell className="text-right">
                          ...
                          >
                            ...
                            <DropdownMenuContent align="end" className="w-44">
                              <DropdownMenuItem onSelect={() => setSelectedDocument(document)}>
                                <Eye />
                                View details
                              </DropdownMenuItem>
                              <DropdownMenuItem onSelect={() => window.open(document.fileUrl, "_blank", "noopener,noreferrer")}>
                                <ExternalLink />
                                Open file
                              </DropdownMenuItem>
                              <DropdownMenuItem
      ...
      <Sheet open={Boolean(selectedDocument)} onOpenChange={(open) => !open && setSelectedDocument(null)}>
        <SheetContent className="sm:max-w-xl">
          <SheetHeader>
            ...
            <SheetDescription>
              Read-only metadata and file information for the selected document.
            </SheetDescription>
          </SheetHeader>
          {selectedDocument && (
            <div className="grid gap-4 p-4">
              <div className="celestial-card tone-surface tone-sapphire p-4">
              ...
              <div className="grid gap-3 text-sm sm:grid-cols-2">
                ...
                <DetailItem label="File type" value={selectedDocument.fileType || "Unknown"} />
                <DetailItem label="Uploaded" value={formatDate(selectedDocument.createdAt)} />
              </div>
